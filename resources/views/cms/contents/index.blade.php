                    ...
                    <tbody class="divide-y divide-slate-100 dark:divide-white/5">
                        @forelse ($contents as $content)
                            <tr class="bg-white/70 text-slate-700 dark:bg-slate-950/30 dark:text-stone-200">
                                <td class="px-5 py-4">
                                    <p class="font-medium text-slate-900 dark:text-white">{{ $content->title }}</p>
                                    <p class="mt-1 text-xs text-slate-500 dark:text-stone-400">{{ \Illuminate\Support\Str::limit($content->summary ?: strip_tags((string) $content->body), 120) }}</p>
                                </td>
                                <td class="px-5 py-4 text-slate-500 dark:text-stone-400">{{ $content->category?->name ?? 'Unassigned' }}</td>
                                <td class="px-5 py-4 text-xs uppercase tracking-[0.15em] text-slate-500 dark:text-stone-400">{{ $content->status }}</td>
                                <td class="px-5 py-4 text-slate-500 dark:text-stone-400">{{ $content->normalizedLayoutType() }}</td>
                                <td class="px-5 py-4">
                                    <div class="flex justify-end gap-2">
                                        ...
                                        @if (auth()->user()?->hasCmsPermission('cms.manage.contents'))
                                            ...
                                            <form method="POST" action="{{ route('cms.contents.destroy', $content) }}">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="inline-flex h-8 w-8 items-center justify-center rounded-full bg-rose-50 text-rose-600 transition hover:bg-rose-100 hover:text-rose-700 dark:bg-rose-500/10 dark:text-rose-400 dark:hover:bg-rose-500/20" title="Delete" onclick="return confirm('Delete this content entry?');">
