...
<div class="mx-auto max-w-7xl px-6 py-10 sm:py-12 lg:px-8">
  @if ($contents->isEmpty())
    <p class="text-slate-500 dark:text-slate-400">No published content yet. Check back soon.</p>
  @else
    <div class="mt-6 divide-y divide-slate-200 dark:divide-slate-800 sm:mt-8">
      @foreach ($contents as $content)
        <a href="{{ route('public.contents.show', $content) }}"
           class="flex flex-col gap-4 py-6 transition hover:text-indigo-600 dark:hover:text-indigo-400 sm:flex-row sm:items-start sm:justify-between sm:gap-6">
          <div class="min-w-0 flex-1">
            ...
            @if ($content->summary)
              <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">{{ \Illuminate\Support\Str::limit($content->summary, 130) }}</p>
            @endif
          </div>
          <svg class="h-5 w-5 shrink-0 text-slate-400 sm:mt-1" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
